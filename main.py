# Performs dataset, vocabulary, transform, and dataloader sanity checks.
...
# Runs basic tests to ensure preprocessing and dataset pipeline function correctly.
def sanity_check():
    ...
    print(f"Total annotation lines: {len(samples)}")

    captions = [cap for _, cap in samples]

    # Builds a vocabulary from captions
    vocab = build_vocab(captions, min_freq=2, max_size=5000)

    # Maps words to IDs
    tokenizer = VocabTokenizer(vocab)

    # Creates text transform with padding and attention mask
    text_transform = get_text_transform(tokenizer, max_len=30)

    # Loads a small sample dataset for checking
    dataset = Flickr30kDataset(
        image_transform=get_image_transform(),
        text_transform=text_transform,
        max_samples=32,
    )
    print("Flickr30kDataset length:", len(dataset))

    # Retrieves a single dataset item
    image, token_ids, attn_mask = dataset[0]
    print("Single sample image shape:", image.shape)
    print("Single sample token_ids shape:", token_ids.shape)
    print("Single sample attn_mask shape:", attn_mask.shape)

    # Builds a dataloader for batching
